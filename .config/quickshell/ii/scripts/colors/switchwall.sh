...
if command -v swww &>/dev/null; then
    current_wallpaper=\$(jq -r '.background.wallpaperPath' "$SHELL_CONFIG_FILE" 2>/dev/null)
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            # Handle static image wallpaper with swww
            missing_deps=()
            if ! command -v swww &> /dev/null; then
                missing_deps+=("swww")
            fi
            ...
            if [ ${#missing_deps[@]} -gt 0 ]; then
                echo "Missing deps for static wallpapers: ${missing_deps[*]}"
                echo "Install swww for better wallpaper support"
                action=$(notify-send \
                    -a "Wallpaper switcher" \
                    -c "im.error" \
                    -A "install_arch=Install swww (Arch)" \
                    "Can't set wallpaper optimally" \
                    "Missing swww. Install for better large image support.")
                if [[ "$action" == "install_arch" ]]; then
                    kitty -1 yay -S swww
                fi
            fi

            # Ensure swww daemon is running and set wallpaper
            ensure_swww_daemon
            if command -v swww &> /dev/null; then
                swww img "$imgpath" --transition-type any --transition-fps 60
            fi

            # For color generation, try to use fallback image if the main one might be too large
            color_extraction_path="$imgpath"
            echo "DEBUG: Original path: $imgpath"
            # Check if this looks like an upscaled image and if original exists
            if [[ "$imgpath" =~ _upscayl_[0-9]+x_|_[0-9]+x|_upscaled ]]; then
                echo "DEBUG: Detected upscaled image pattern"
                fallback_path=$(echo "$imgpath" | sed -E 's/_upscayl_[0-9]+x_[^.]*|_[0-9]+x|_upscaled//')
                echo "DEBUG: Fallback path: $fallback_path"
                if [[ -f "$fallback_path" ]]; then
                    echo "Using fallback image for color extraction: $fallback_path"
                    color_extraction_path="$fallback_path"
                else
                    echo "DEBUG: Fallback path does not exist"
                fi
            else
                echo "DEBUG: No upscaled pattern detected"
            fi
            echo "DEBUG: Final color extraction path: $color_extraction_path"

            matugen_args=(image "$color_extraction_path")
            generate_colors_material_args=(--path "$color_extraction_path")
